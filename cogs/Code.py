...
class Code(commands.Cog):
    ...
    async def run(self, ctx, language, *, content):
        ...
        try:
            ...
            await ctx.send(embed=quick_embed(f"Language: **{ans['language'].upper()}**\nVersion: **{ans['version']}**\n\nOutput:\n```{ans['run']['output']}```"))
        except Exception:
            await ctx.send(embed=quick_embed("Something went wrong! Check the input format of the code.", False))
