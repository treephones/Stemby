...
async def get_flashcard(topic):
    fcards = requests.post("https://fastcardsapi.herokuapp.com/api/flashcards", data={"query": topic, "max": 10}).json()
    chosen = choice(fcards['flashcards'])
    return chosen['question'], chosen['answer'], fcards['from_url']
