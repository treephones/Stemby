...
async def graph(expression, person):
    og_expression = expression
    expression = expression.replace("^", "**").replace(" ", "")
    matches = re.findall("\d+x", expression)
    for match in matches:
        expression = expression.replace(match, f"{match[:-1]}*{match[-1]}")
    h = max(map(float, re.findall("[+-]?\d+(?:\.\d+)?", expression)))*2
    l, r = expression.replace("^", "**").split("=")
    l, r = l.strip(), r.strip()
    #edit ranges
    plot = plot_implicit(parse_expr(l) - parse_expr(r), x_var=("x", -1000, 1000), y_var=("y", -1000, 1000), show=False, points=600)
